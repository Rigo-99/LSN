...
int main (int argc, char *argv[]){
	...
	int numero_riscalature = 9;
	int step_per_block = 300;
	int step_per_riscalatura = 125;
	bool restart = false;
	...
	if ( argc==5 ) {
		...
		cout << endl;
		cout << "Utilizzo valori in parentesi quadre:\n./es04_1.exe " << numero_riscalature << " " << step_per_riscalatura << " " << 0 << " " << step_per_block << "\n\n";
...
void Measure(bool in_blocchi){ //Properties measurement
	...
	media_pot = ((media_pot * len_block) + stima_pot)/(len_block+1.);
	media_kin = ((media_kin * len_block) + stima_kin)/(len_block+1.);
	media_etot = ((media_etot * len_block) + stima_etot)/(len_block+1.);
	media_temp = ((media_temp * len_block) + stima_temp)/(len_block+1.);
	...
	if (in_blocchi){
		blocchi[iv][i_block] = media_pot;
		blocchi[ik][i_block] = media_kin;
		blocchi[ie][i_block] = media_etot;
		blocchi[it][i_block] = media_temp;
...
void Salva_Blocchi(void){				//Write final configurations
	
	const char* nomi_files[] = {"output_epot_blocchi.dat","output_ekin_blocchi.dat","output_etot_blocchi.dat","output_temp_blocchi.dat"};
	
	
	Print_media_sigma_blocchi_multi(blocchi, i_block, n_props, nomi_files);
