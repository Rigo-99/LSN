...
int main()
{
	...
	for(int iblk=1; iblk <= nblk; ++iblk){ //Simulation
		...
		for(int istep=1; istep <= nstep; ++istep){
			...
			double pressione = rho * temp + (walker[iw] + ptail * (double)npart) / vol;
			//see << walker[iv]/npart << "\t" << pressione << endl;
...
void Measure()
{
	...
	for (int i=0; i<npart-1; ++i){
		for (int j=i+1; j<npart; ++j){
			...
			int ind_dist = int(double(dr)/double(bin_size));
			
			if (dr < 0.5*box){
				walker[igofr+ind_dist] += 2.;
			}
...
void Averages(int iblk) //Print results for current block
{
	...
	for (int i=igofr; i<n_props; i++){
		double den = rho*npart*(4./3.)*M_PI*(pow((i-igofr+1.5)*bin_size,3.)-pow((i-igofr+0.5)*bin_size,3.));
		double stima = blk_av[i]/double(blk_norm)/den;
		glob_av[i] += stima;
		glob_av2[i] += pow(stima,2);
...
void ConfFinal(void)
{
	...
	for (int i=igofr; i<n_props; i++){
		double err=Error(glob_av[i],glob_av2[i],nblk);
		Gave << setw(wd) << (i-igofr+0.5)*bin_size << setw(wd) << glob_av[i]/(double)nblk << setw(wd) << err << endl;
	}
	Gave.close();
}
